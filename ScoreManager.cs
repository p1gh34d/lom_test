...
private void HandleNoteSustainEnd(int midi)
{
    ...
    if (activeSustainedSpNoteController != null && activeSustainedSpNoteController.Midi == midi && activeSustainedSpNoteData != null)
    {
        ...
        // Ранний отпуск — считаем секцию неуспешной
        if (currentTime + 1e-3f < naturalEnd)
        {
            int sectionIndex = ChartParser.StarPowerSections.FindIndex(sp =>
...
private void Update()
{
    ...
    var snapshot1 = noteSpawner.SpawnedObjects.ToArray();
    foreach (var noteObj in snapshot1)
    {
    ...
    {
        var spawnedSnapshot = noteSpawner.SpawnedObjects.ToArray();
        foreach (var noteObj in spawnedSnapshot)
        {
            if (noteObj == null) continue;
            NoteController note = noteObj.GetComponent<NoteController>();
